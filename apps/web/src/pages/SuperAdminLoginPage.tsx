...
// Pas de sous-domaine ici (contrairement à LoginPage) : un SUPER_ADMIN
// n'appartient à aucun tenant — voir auth.service.ts#loginSuperAdmin.
export function SuperAdminLoginPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setEnCours(true);
    try {
      await loginSuperAdmin(email, motDePasse);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface-container-low px-4">
      <div className="w-full max-w-sm bg-surface border border-outline-variant rounded-xl p-8 shadow-sm">
        ...
        <p className="text-sm text-on-surface-variant mb-6">Console Super-Admin</p>
